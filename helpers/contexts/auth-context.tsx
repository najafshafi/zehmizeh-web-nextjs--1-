...
// NOTE: When sending "withCredentials" in requests, the API must specify an exact origin in "allowed-origin" response.
// Wildcards won't work, so adjust the server's CORS settings to avoid errors.
const client = axios.create({
  baseURL: BASE_URL,
  withCredentials: true,
});
...
function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const intercomHandler = () => {
    ...
    if (typeof window !== 'undefined' && window.location.host.includes('beta')) {
      customAttributes.platform = 'Beta';
    } else {
      customAttributes.platform = 'Live';
    }
