...
// Safely get the useIntercom hook with proper types
const getUseIntercom = (): (() => Partial<IntercomContextValues>) => {
  try {
    // Import the hook
    return require("react-use-intercom").useIntercom;
  } catch (error) {
    // Return dummy implementation if module isn't available
    return () => dummyIntercom;
  }
};

// Get the useIntercom hook
const useIntercom = getUseIntercom();
...
function AuthProvider({ children }: { children: React.ReactNode }) {
  // Create internal QueryClient just for the auth context
  ...
  // Create the actual provider content
  const AuthProviderContent = () => {
    const [user, setUser] = React.useState<any>(null);
    const [isLoading, setIsLoading] = React.useState(false);
    const [isBoostraping, setIsBootstraping] = React.useState(true);

    // Safely use Intercom with a fallback
    let intercomUtils: Partial<IntercomContextValues>;
    try {
      intercomUtils = useIntercom();
    } catch (error) {
      intercomUtils = dummyIntercom;
    }

    const { boot, shutdown } = intercomUtils;
