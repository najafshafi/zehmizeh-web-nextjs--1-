...
export const validateProposal = yup
  .object({
        ...
        test: function (value, context) {
          if (value < 5) {
            return this.createError({
              message: `${
                context.parent.isHourlyJob
                  ? "Hourly rate must be at least $5."
                  : "Project fee must be at least $5."
              }`,
            });
          } else {
            return true;
    ...
    estimation: yup
      .object({ number: yup.string(), duration: yup.string() })
      .test({
    ...
    termsAndConditions: yup
      ...
      .test({
        message: "Maximum 2000 characters are allowed",
        test: (value) => (value ? getPlainText(value).length <= 2000 : true),
      }),
    questions: yup
      .string()
      .optional()
...
export const validateCourse = yup
  .object({
    ...
    school_name: yup
      .string()
      .required("Please enter from where did you do course"),
  })
  .required();

export const validateDispute = yup
  .object({
    selectedPeojectId: yup
      .string()
      .nullable()
      .required("Please select a project."),
    selectedMilestoneId: yup
      .string()
      .nullable()
...
interface ValidationFields {
  accountHolderFirstName: yup.StringSchema;
  accountHolderLastName: yup.StringSchema;
  accountNumber: yup.StringSchema;
  accountHolderType: yup.StringSchema;
  routingNumber?: yup.StringSchema;
  transitNumber?: yup.StringSchema;
  institutionNumber?: yup.StringSchema;
  [key: string]: yup.Schema | undefined;
}

export const addBankAccountValidationHandler = (
  country: string,
  type?: string
) => {
  const validationFeilds: ValidationFields = {
    ...
    routingNumber: yup
      .string()
      .test({
        name: "routingNumberValidation",
        message: "Please enter the routing number",
        test: function (value) {
          // Skip validation if country is CA
    ...
    transitNumber: yup
      .string()
      .test({
        name: "transitNumberValidation",
        message: "Please enter the transit number",
        test: function (value) {
          // Skip validation if country is not CA
          if (country !== "CA") return true;
    ...
    institutionNumber: yup
      .string()
      .test({
        name: "institutionNumberValidation",
        message: "Please enter the institution number",
        test: function (value) {
          // Skip validation if country is not CA
          if (country !== "CA") return true;
          if (!value) return false;
  ...
  return yup.object(validationFeilds as any).required();
};
...
export const fetchCountriesCode = async () => {
  try {
    ...
    return data.map((dt: CountryData) =>
      dt.country_short_name.toLocaleLowerCase()
    );
  } catch (error) {
    return [];
  }
...
export const paymentProcessingStatusHandler = (
  method: "ACH" | "OTHER" = "OTHER"
) => {
  const status = {
