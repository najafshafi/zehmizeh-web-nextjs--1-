...
const CustomUploader = ({
  ...
  */
  ...
  return (
    <Wrapper>
      <label
        className={`file-uploader w-100 fs-1rem fw-300 pointer d-flex ${uploading ? 'uploading' : ''}`}
        htmlFor="file-upload"
      >
      ...
      <input
        ...
        type="file"
        className="w-100"
        onChange={(e) => (multiple ? onChangeMultipleFile(e) : onChangeSingleFile(e))}
        disabled={uploading}
      ...
      <UploadCount>
        <div className="upload-file-types w-100 fs-1rem fw-300 mt-2">
          {suggestions || 'File type: PDF, DOC, DOCX, XLS, XLSX, JPG, PNG, JPEG'}
        </div>
        {limit && (
      ...
      {attachments?.length > 0 && (
        <div className="uploads-attached d-flex flex-wrap mt-3 gap-2">
