...
import { emailPattern, onlyCharacters, validatePhoneNumber } from '@/helpers/validation/common';
...
export const AccountDetails = ({ setStep, setPayload, shouldShow }: Props) => {
  ...
  return (
    <Wrapper>
      ...
      {!isClient && (
        <>
          ...
          <p className="fs-14" style={{ color: '#686868' }}>
            A freelancer who works alone is not considered an agency. If this account will be shared between coworkers,
            register as an agency.
          </p>
          <div className="d-flex mt-4 agency">
            <button
              type="button"
              id="y"
              className={agency ? 'me-4 text-start option-button active-button' : 'me-4 text-start option-button'}
            ...
            <button
              type="button"
              id="n"
              className={!agency ? 'text-start option-button active-button' : 'text-start option-button'}
      ...
      <h2 className="mb-3 fs-24 fw-300">Account details can be changed later</h2>
      <Row className="register--form-row">
        ...
        <Col xs={12} lg={isCountryHaveState ? 6 : 12}>
          <CountryDropdown selectedCountry={location} onSelectCountry={onSelectCountry} />
        ...
        <Col lg={12}>
          <PhoneInputWrapper>
            ...
            <PhoneNumberInput initialValue={phoneInput.formattedPhoneNumber} onChange={handlePhoneChange} />
          </PhoneInputWrapper>
        ...
        <Col lg={12}>
          <FloatingLabel controlId="floatingInput" label="Password" className="password-input">
            <Tooltip customTrigger={<InfoIcon />} className="password-tooltip">
              Every password must include at least: 1 uppercase letter, 1 lowercase letter, 1 number, 1 symbol, and at
              least 8 characters
            </Tooltip>
            <span className="pointer" onClick={() => togglePasswordPreview('password')}>
              <Eye
        ...
        <Col lg={12}>
          <FloatingLabel controlId="floatingInput" label="Confirm Password" className="password-input">
            <span className="pointer" onClick={() => togglePasswordPreview('password')}>
              <Eye
                className={cns('input-icon', {
                  active: passwordPreview.password,
      ...
      <div className="d-flex flex-row justify-content-between">
        <StyledButton disabled={verifyingEmailPhone} variant="secondary" onClick={() => setStep(1)}>
          Back
        </StyledButton>
        <StyledButton disabled={verifyingEmailPhone} onClick={handleSubmit(handleNext)}>
