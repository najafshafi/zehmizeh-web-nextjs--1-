...
const Applicants = ({ jobPostId, refetch, jobStatus, jobTitle }: Prop) => {
  ...
  const handleAcceptTerms = async () => {
    ...
    const { conversation } = await talkJsFetchSingleConversation(`proposal_${chat_proposal_id}`);
    toast.remove();

    setFreelancerName(`${pendingProposal.first_name} ${pendingProposal.last_name}`);
  ...
  return (
    <>
      <Sorting sortTypes={sortTypes} sorting={sortKey} setSorting={setSorting} />
      <ApplicantsWrapper className="d-flex flex-column">
        ...
        {!isLoading && data?.length == 0 && (
          <NoDataFound
            className="py-5"
            title={jobStatus !== 'active' ? 'No one has applied yet' : 'There are no declined proposals to view.'}
          />
        )}
        ...
        {!isLoading &&
          data?.length > 0 &&
          data?.map((item: any) => (
            <ApplicantListItem key={item?.proposal_id} className="d-flex flex-wrap justify-content-between">
                ...
                <div className="main-details d-flex flex-column flex-1">
                  <div className="d-flex justify-content-between">
                    <div className="d-flex align-items-center gap-2">
                      ...
                      <StatusBadge className="width-fit-content" color={item?.is_viewed ? 'green' : 'red'}>
                        {item?.is_viewed ? 'Read' : 'Unread'}
                      </StatusBadge>
                      {item?.is_agency ? <StatusBadge color="blue">Agency</StatusBadge> : null}
                    </div>
                  ...
                  <div className="d-flex justify-content-between align-items-end">
                    <div>
                      <div className="description fs-18 fw-400 light-text line-height-140 capital-first-ltr mb-2">
                        {item?.job_title}
                      </div>
                      {item?.date_created && (
