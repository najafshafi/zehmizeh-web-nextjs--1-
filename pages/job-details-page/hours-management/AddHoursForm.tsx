...
const AddHoursForm = ({ show, toggle, onSubmit, jobPostId, selectedMilestone, hourlyRate, isFinalHours }: Props) => {
  ...
  useEffect(() => {
    if (show && selectedMilestone) {
      ...
      if (selectedMilestone?.attachments) {
        const attachments: { fileUrl?: string; fileName?: string }[] = selectedMilestone?.attachments
          ?.split(',')
          .map((dt: string) => {
            return {
  ...
  const validationSchema = yup
    .object({
      ...
      amount: yup
        .number()
        .required()
        .test('Is positive?', 'Please enter a number greater than 5', (value) => value > 4.99 && value < 99999)
        .typeError('Please enter a valid number value'),
  ...
  const validate = (e: any) => {
    e.preventDefault();
    validationSchema.isValid({ hoursWorked, title, amount, description }).then((valid) => {
      if (!valid) {
        validationSchema.validate({ hoursWorked, title, amount, description }, { abortEarly: false }).catch((err) => {
  ...
  const submitHours = () => {
    ...
    toast.promise(promise, {
      ...
      error: (err) => {
        setLoading(false);
        return selectedMilestone ? err.response : err?.response?.data?.message || 'error';
      },
    });
  };
  ...
  const handleUploadImage = ({ file, fileName }: { file: string; fileName?: string }) => {
    handleChange('screenshot_link', [...formState.screenshot_link, { fileUrl: file, fileName }]);
  };
  ...
  const handleMultipleUploadImage = (files: { file: string; fileName?: string }[]) => {
    const newUploadedFiles = files.map(({ file, fileName }) => {
      return { fileUrl: file, fileName };
    });

    handleChange('screenshot_link', [...formState.screenshot_link, ...newUploadedFiles]);
  };

  return (
    <StyledModal maxwidth={570} show={show} size="sm" onHide={closeModal} centered>
      <Modal.Body>
        ...
        <FormWrapper onSubmit={validate}>
          <div className="fs-24 fw-400">
            {selectedMilestone ? 'Edit Hours' : isFinalHours ? 'Submit Final Hours' : 'Add Hours'}
          </div>
          <StyledFormGroup>
          ...
          <StyledFormGroup>
            ...
            <Form.Control
              ...
              className="form-input"
            />
            {errors?.hoursWorked && <ErrorMessage message={errors?.hoursWorked} />}
          </StyledFormGroup>
          <StyledFormGroup>
          ...
          <StyledFormGroup>
            <FormLabel className="fs-1rem fw-300">
              Attach any completed work below. (Even research, notes, or incomplete drafts may be worth sharing.)
            </FormLabel>
            <CustomUploader
              handleUploadImage={handleUploadImage}
              attachments={formState?.screenshot_link ? formState?.screenshot_link : []}
              ...
              multiple
            />
            {errors?.screenshot_link && <ErrorMessage message={errors.screenshot_link} />}
          </StyledFormGroup>
