...
const MessageDate = styled.div<{ isFromSingleMessaging: boolean }>`
  position: sticky;
  top: 10px;
  display: inline;
  margin: 14px;
...
const Conversation = ({
  ...
  const searchChatHandler = () => {
    ...
    const message = document.getElementById(
      `single-message-${searchedChat.chat_id}`
    );
  ...
  const isProposalReopenedText = useMemo(() => {
    if (activeChat?.status_change_timestamp?.pending_date) {
      if (user.user_type === "client") {
        return `You reopened this proposal - ${moment(
  ...
  if (showMilestoneAlert)
    return (
      <Wrapper id="message-container">
        <div className="p-7 text-base">
          {user.user_type === "freelancer" ? (
            <p>
              Before submitting your first milestone or hours submission,
              message your client on the project details page -{" "}
              <Link
                style={{ color: "#f2b420" }}
                href={`/job-details/${activeChat?._job_post_id}/messages`}
              >
                {" "}
                here.
              </Link>
            ...
            <p>
              Before the freelancer submits the first milestone or hours
              submission, message them on the project details page -{" "}
              <Link
                style={{ color: "#f2b420" }}
                href={`/client-job-details/${activeChat?._job_post_id}/messages`}
              >
                {" "}
                HERE.
              </Link>
                  ...
                  return (
                    <React.Fragment key={`message-${message.chat_id}-${index}`}>
                      ...
                      {(message?.message_text || "").includes(
                        "Project proposal has been reopened"
                      ) &&
                        (message?.message_text || "").includes(
                          "Message from ZMZ Admin"
                        ) &&
                        isProposalReopenedText && (
                          <div className="message-limit-note-warn mb-3">
                            {isProposalReopenedText}
                          </div>
                        )}
