...
const AddBankAccount = ({ show, onClose, onUpdate }: Props) => {
  ...
  const validateForm = () => {
    ...
    const validateEq = addBankAccountValidationHandler(country, formState.accountHolderType);
  ...
  const addBankAccount = () => {
    ...
    const body = {
      action: 'add_account',
      account_holder_name:
        camelCaseToNormalCase(accountHolderFirstName) + ' ' + camelCaseToNormalCase(accountHolderLastName),
    ...
    if (institutionNumber && transitNumber) body['routing_number'] = `${transitNumber}-${institutionNumber}`;
  ...
  return (
    <StyledModal maxwidth={678} show={show} size="sm" onHide={onModalClose} centered>
      <Modal.Body>
        ...
        <div>
          ...
          <div className="d-flex align-items-center">
            Add Details Below&nbsp;
            <Tooltip>
              To get paid on ZMZ, add the details of the bank account you would like your fees to be sent to. These
          ...
          <div className="account-form">
            <StyledFormGroup>
              ...
              {errors?.accountHolderType && <ErrorMessage message={errors.accountHolderType} />}
            </StyledFormGroup>
            ...
            <Row>
              <Col>
                <StyledFormGroup>
                  <Form.Control
                    placeholder={formState?.accountHolderType === 'company' ? 'First Name/Business Name' : 'First Name'}
                    className="form-input"
                    value={formState?.accountHolderFirstName}
                    onChange={(e) => handleChange('accountHolderFirstName', e.target.value)}
                    maxLength={60}
                  />
                  {errors?.accountHolderFirstName && <ErrorMessage message={errors.accountHolderFirstName} />}
                </StyledFormGroup>
              </Col>
              <Col>
                <StyledFormGroup>
                  <Form.Control
                    placeholder={formState?.accountHolderType === 'company' ? 'Last Name (Optional)' : 'Last Name'}
                    className="form-input"
                    value={formState?.accountHolderLastName}
                    onChange={(e) => handleChange('accountHolderLastName', e.target.value)}
                    maxLength={60}
                  />
                  {errors?.accountHolderLastName && <ErrorMessage message={errors.accountHolderLastName} />}
                </StyledFormGroup>
              </Col>
            </Row>
            ...
            <StyledFormGroup>
              <Form.Control
                ...
                onChange={(e) => handleChange('accountNumber', e.target.value.trim())}
                maxLength={30}
              />
            ...
            {!['IL', 'GB', 'BE', 'MX', 'CA', 'AR', 'CH'].includes(auth?.user?.preferred_banking_country) && (
              <StyledFormGroup>
                <Form.Control
                  ...
                  maxLength={20}
                  onChange={(e) => handleChange('routingNumber', e.target.value.trim())}
                />
                {errors?.routingNumber && <ErrorMessage message={errors.routingNumber} />}
              </StyledFormGroup>
            )}
                  ...
                  <Col>
                    <StyledFormGroup>
                      <Form.Control
                        ...
                        maxLength={20}
                        onChange={(e) => handleChange('transitNumber', e.target.value.trim())}
                      />
                      {errors?.transitNumber && <ErrorMessage message={errors.transitNumber} />}
                    </StyledFormGroup>
                  </Col>
                  <Col>
                    <StyledFormGroup>
                      <Form.Control
                        ...
                        maxLength={20}
                        onChange={(e) => handleChange('institutionNumber', e.target.value.trim())}
                      />
                      {errors?.institutionNumber && <ErrorMessage message={errors.institutionNumber} />}
                    </StyledFormGroup>
                  </Col>
                </>
              )}
