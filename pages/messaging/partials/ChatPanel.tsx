...
function ChatPanel({ ignoreMessageLoading, showMilestoneAlert = true, isFromSingleMessaging = false }: any) {
  ...
  const { user } = useAuth();
  const { loading, activeChat, activeTab, errors, chatList } = useSelector((state: RootState) => state.chat);
  const remoteUser =
  ...
  const isProposalConversation =
    activeTab === 'proposals' && activeChat?.job_status === 'prospects' && activeChat?.proposal_status === 'pending';

  const isInviteConversation =
    activeTab === 'invities' && activeChat?.job_status === 'prospects' && activeChat?.invite_status === 'read';

  const todaysDate = moment();
  const jobClosedDate = activeChat?.job_end_date ? moment(activeChat?.job_end_date) : moment();
    ...
    if (user?.user_type === 'client' && activeTab === 'proposals') {
      return (
        ...
        <span style={{ color: 'blue' }}>
          ...
          <br />
          ‘Proposal’ chats are only meant to help you pick a freelancer - <b>not to work on projects.</b>
          <br />
          To hire this freelancer, click the ‘Accept Proposal’ button on their proposal{' '}
          <Link
      ...
      return (
        ...
        // </span>
        <span style={{ color: 'blue' }}>
          <b>NOTE: DO NOT SUBMIT WORK IN THIS CHAT.</b> You have NOT been hired to this project (and cannot be paid on
          ZMZ) until the client accepts your proposal.
        </span>
      );
    }
  ...
  const getProposalId = (jobPostId: string, remoteUserId: string) => {
    const proposal = chatList['proposals'].find((chat) => {
      const condOne = chat._from_user_data.user_id === remoteUserId && chat._to_user_data.user_id === user.user_id;
      const condTwo = chat._from_user_data.user_id === user.user_id && chat._to_user_data.user_id === remoteUserId;
  ...
  if (activeTab === 'proposals') {
    if (user?.user_type === 'freelancer') {
      backToProposalInviteJobButton.onClick = () => navigate(`/job-details/${activeChat._job_post_id}/proposal_sent`);
    ...
    if (user?.user_type === 'freelancer') {
      ...
      backToProposalInviteJobButton.onClick = () => navigate(`/offer-details/${activeChat._job_post_id}`);
    } else {
      backToProposalInviteJobButton.text = 'Go to Invites';
      backToProposalInviteJobButton.onClick = () => navigate(`/client-job-details/${activeChat._job_post_id}/invitees`);
    }
  } else {
    if (user?.user_type === 'freelancer') {
      backToProposalInviteJobButton.text = 'Go to Project';
      backToProposalInviteJobButton.onClick = () => navigate(`/job-details/${activeChat._job_post_id}`);
    } else {
      backToProposalInviteJobButton.text = 'Go to Project';
      backToProposalInviteJobButton.onClick = () => navigate(`/client-job-details/${activeChat._job_post_id}`);
    }
  }
    ...
    if (activeTab === 'proposals') {
      switch (activeChat?.job_status) {
        case 'deleted': {
          ...
          if (user?.user_type === 'freelancer')
            return <span>The client canceled this project post - {formatDate(activeChat.date_modified)}</span>;
          return <span>You canceled this project post - {formatDate(activeChat.date_modified)}</span>;
        }
        case 'closed':
        case 'active': {
          ...
          if (user?.user_type === 'freelancer' && activeChat?.proposal_status === 'accept') {
            return (
              <span>
                The client accepted your proposal on {formatDate(activeChat.proposal_modified_date)}! This conversation
            ...
            return (
              <span>
                When you accepted this freelancer's project proposal on {formatDate(activeChat.proposal_modified_date)},
        ...
        case 'prospects': {
          ...
          if (user?.user_type === 'freelancer')
            return (
              <span>
                The client has declined your project proposal - {formatDate(activeChat.proposal_modified_date)}
              </span>
            );
          return <span>You declined this proposal - {formatDate(activeChat.proposal_modified_date)}</span>;
        }
        default:
          return undefined;
      }
      ...
      switch (activeChat?.job_status) {
        ...
        case 'prospects': {
          ...
          if (user?.user_type === 'freelancer' && activeChat?.invite_status == 'accepted') {
            return (
              <span>
                When you submitted a proposal to this project {formatDate(activeChat.invite_modified_date)}, this
                conversation was moved to the "Proposals" tab{' '}
                <Link
                  style={{ textDecoration: 'underline' }}
                  className="link"
                  to={`/messages-new/proposal_${getProposalId(activeChat._job_post_id, remoteUser?.user_id)}`}
                >
                  HERE.
                </Link>{' '}
            ...
            return (
              <span>
                The freelancer submitted a proposal to this project on {formatDate(activeChat.invite_modified_date)}, so
            ...
            return <span>The client has canceled your invitation - {formatDate(activeChat.invite_modified_date)}</span>;
          } else {
            return <span>You have canceled the invitation. - {formatDate(activeChat.invite_modified_date)}</span>;
          }
        }
        default:
          return undefined;
  ...
  if (!activeChat || ((loading.message || loading.list) && !ignoreMessageLoading)) {
    return (
    ...
    <ChatPanelWrapper className="m--chatpanel">
      ...
      >
        <div className="m--chatpanel-activeUser">
          <div>
            {!isDesktop ? <BackArrow style={{ cursor: 'pointer' }} onClick={backToUserList} /> : null}
            <h5 className="text-capitalize">{getUserName(remoteUser)}</h5>
            <p className="m--chatheader-job-title"> {activeChat?.job_title || ''}</p>
          </div>
        </div>
        {isFromSingleMessaging ? (
          ...
          backToProposalInviteJobButton.text && (
            <div className="d-flex gap-3">
              <div>
                <TimezoneUI isFromSingleMessaging={isFromSingleMessaging} remoteUser={remoteUser} />
              </div>
              <div>
                <ChatHeaderButton
                  variantType={'primary'}
      ...
      <div className="d-block d-lg-none">
        <SearchMessages value={searchTerm} onClick={toggleSearchModal} onClear={onClearSearch} />
      </div>
      <section className="m--chatpanel-body">
        {/* If the job is closed, allow freelancer or client to chat for 2 weeks ------------ start */}

        {activeChat && activeChat.job_status === 'closed' && activeChat.job_end_date && daysOfJobClosed < 14 && (
          <div id="message-limit-note" className="message-limit-note">
            <span style={{ color: 'blue' }}>
              ...
              {moment(activeChat.job_end_date).add(14, 'days').format('MMM DD, YYYY')}
            </span>
          </div>
        )}
        ...
        */}
        ...
        <Conversation
          ...
          showMilestoneAlert={activeTab === 'jobs' && activeChat?.total_milestones === 0 && showMilestoneAlert}
          isFromSingleMessaging={isFromSingleMessaging}
        />
        {sendMessageDisabledText ? (
          <div className="message-limit-note-warn">{sendMessageDisabledText}</div>
        ) : (
          <>
            ...
            */}
            {activeTab === 'proposals' || activeTab === 'invities' ? (
              <>
                {activeChat.job_status === 'prospects' && activeChat?.proposal_status === 'pending' && (
                  <CreateMessage conversationId={`proposal_${activeChat?.proposal_id}`} />
                )}

                {activeChat.job_status === 'prospects' && activeChat?.invite_status === 'read' && (
                  <CreateMessage conversationId={`invite_${activeChat?.invite_id}`} />
                )}
              </>
            ) : (
              <>
                {(isFromSingleMessaging ||
                  !(activeTab === 'jobs' && activeChat?.total_milestones === 0 && showMilestoneAlert)) && (
