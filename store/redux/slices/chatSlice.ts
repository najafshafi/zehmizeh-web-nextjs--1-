...
export const fetchChatList = createAsyncThunk<any, { cancelToken: CancelToken }>(
  'chats/fetch-chat-list',
  async ({ cancelToken }, { rejectWithValue }) => {
    try {
      const { data, status }: AxiosResponse<{ data: I.ChatListAPIResponse }> = await apiClient.post(
...
export const selectChatHandler = createAsyncThunk<
  any,
  { chatItem: I.Invite & I.Proposal & I.Job; index: number; remoteUserId: string }
>('chats/fetch-messages', async ({ chatItem, index, remoteUserId }, { dispatch }) => {
  try {
    dispatch(updateSelectChat({ chatItem, index }));

    const { data }: AxiosResponse<{ data: I.MessagesApiResponse }> = await apiClient.post('/message/manage-message', {
      action: 'get_messages',
...
export const addNewMessage = createAsyncThunk<any, { message: I.AddMessagePayload }>(
  '/chats/add-message',
  async ({ message }, { rejectWithValue }) => {
    try {
      message.action = 'add_message';

      const { data: new_message }: AxiosResponse<{ data: I.MessagesApiResponse }> = await apiClient.post(
        '/message/manage-message',
        message
      );
...
const chatSlice = createSlice({
  ...
  reducers: {
    ...
    onTabChange: (state, { payload }: PayloadAction<keyof I.UnreadMessages>) => {
    ...
    onParamKeyHandler: (state, { payload }: PayloadAction<{ tab: keyof I.ChatList }>) => {
      state.activeTab = payload.tab;
    },

    updateSelectChat: (
      state,
      { payload }: PayloadAction<{ chatItem: I.Invite & I.Proposal & I.Job; index: number }>
    ) => {
      if (state.activeChat !== payload.chatItem) state.activeChat = payload.chatItem;
    },
    updateMesssageTab: (state, { payload }: PayloadAction<{ updatedTabs: I.TabObject[] }>) => {
      state.tabs = payload.updatedTabs;
    },
    onNewMessage: (state, { payload }: PayloadAction<{ newMessage: I.PusherNewMessage }>) => {
      ...
      if (activeChat !== null) {
        ...
        */
        if (message.tab === 'jobs' && message._job_post_id === activeChat._job_post_id) {
          appendMessage = true;
        }

        if (message.tab === 'proposals' && message.proposal_id === activeChat.proposal_id) {
          appendMessage = true;
        }

        if (message.tab === 'invities' && message.invite_id === activeChat.invite_id) {
          appendMessage = true;
        }
      }
      ...
      if (appendMessage) {
        ...
        tabKeys.forEach((tab) => {
          const index = state.chatList[tab].findIndex((chat: I.Invite & I.Proposal & I.Job) => {
            if (tab === 'jobs') return chat._job_post_id === message._job_post_id;

            if (tab === 'invities')
              return chat._job_post_id === message._job_post_id && chat?.invite_id === message?.invite_id;

            if (tab === 'proposals')
              return chat._job_post_id === message._job_post_id && chat?.proposal_id === message?.proposal_id;
          });

          if (state.chatList[tab][index]) state.chatList[tab][index].unread_count += 1;
        });
      }

      setTimeout(() => scrollToBottom('message-container'), 10);
    },
    onSeenMessage: (state, { payload }: PayloadAction<I.SeenMessageResponse>) => {
      let flag = false;
      const { activeChat } = state;

      if (activeChat) {
        if (activeChat._job_post_id === payload.job_post_id) {
          if (!payload.invite_id && !payload.proposal_id && payload.job_post_id === activeChat._job_post_id) {
            flag = true;
          }

          if (payload.proposal_id && activeChat.proposal_id === payload.proposal_id) {
            flag = true;
          }
    ...
    appendNewMessage: (state, { payload }: PayloadAction<I.AddMessagePayload>) => {
      payload.chat_id = payload.custom_chat_id;
      delete payload.custom_chat_id;
    ...
    replaceChatId: (state, { payload }: PayloadAction<I.ReplaceChatId>) => {
      ...
      const index = state.messages.findIndex((mess) => mess.chat_id === custom_chat_id);
      if (state.messages[index]) state.messages[index].chat_id = chat_id;
    },

    deleteChatFromMessages: (state, { payload }: PayloadAction<{ chat_id: number }>) => {
      state.messages = state.messages.filter((ct) => ct.chat_id !== payload.chat_id);
    },
    searchChatList: (state, { payload }: PayloadAction<{ searchTerm: string }>) => {
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchChatList.fulfilled, (state, { payload }: PayloadAction<I.ChatListAPIResponse>) => {
    ...
    builder.addCase(
      ...
      (state, { payload }: PayloadAction<{ messages: I.ChatMessage[]; index: number }>) => {
        const totalReadMessages = state.chatList[state.activeTab][payload.index].unread_count;
        if (typeof totalReadMessages === 'number') state.unreadMessages[state.activeTab] -= totalReadMessages;
    ...
    builder.addCase(
      ...
      (state, { payload }: PayloadAction<{ new_message: I.AddMessageResponse }>) => {
